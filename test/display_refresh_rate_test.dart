...
void main() {
  test('Android must not use RefreshRate.boost (it resets the window to 60 Hz)', () {
  ...
  test('Android must not rewrite WindowManager.LayoutParams after launch', () {
    expect(DisplayRefreshPolicy.usesPluginWindowHints(isAndroid: true), isFalse);
    expect(DisplayRefreshPolicy.usesPluginWindowHints(isAndroid: false), isTrue);
  });
  ...
  test('already at peak must not recover-vote (ALWAYS would hop 120→30→120)', () {
    expect(
